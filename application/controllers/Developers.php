...
class Developers extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('frontend/developers_model');
	...
	{
		$config = [
            'base_url' => base_url('developers/index'),
	...
	public function detail($id)
	{
		...
		$data["recent_for_sales"] = $this->developers_model->recent_for_sale($id);
		$data["recent_for_rents"] = $this->developers_model->recent_for_rent($id);
		$data["recent_for_new_properties"] = $this->developers_model->recent_for_new_properties($id);


		$data["detail"] = $this->developers_model->find_by_id($id);

		$this->load->view('developers/detail', $data);
	}
