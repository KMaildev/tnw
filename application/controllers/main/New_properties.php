...
class New_properties extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model('admin/new_properties_model');
		...
		{
	        $config = [
	            'base_url' => base_url('main/new_properties/index'),
	            'total_rows' => $this->new_properties_model->count_all(),
	            'per_page' => 50,
	        ...
	        $data["for_sales"] = $this->new_properties_model->getAll($config['per_page'], $offset);
	        $data["count_all"] = $this->new_properties_model->count_all();
			$this->load->view('admin/new_properties/index', $data);
		}
    ...
    public function create()
    {
        ...
        $this->load->view('admin/new_properties/create', $data);
    }
    ...
    public function save() {
        ...
        if ($this->form_validation->run() === FALSE){
            ...
            $data["insert_id"] =  $this->new_properties_model->insert($data);
            $this->session->set_flashdata('success', 'Added Successfully');
            $this->load->view('admin/new_properties/upload_file', $data);
        }
    ...
    {
        $data["detail"] = $this->new_properties_model->getDataById($id);
        $data["photos"] = $this->new_properties_model->getDataByPhoto($id);
				$data["company_data"] = $this->company_model->get_by_company_data();
        $this->load->view('admin/new_properties/detail', $data);
    }


    public function destroy($id)
    {
        $this->new_properties_model->destroy_properties($id);
        $this->new_properties_model->destroy_propertie_photo($id);
        redirect(base_url('main/new_properties'));
    }
    ...
    public function edit($id)
    {
        ...
        $data["detail"] = $this->new_properties_model->getDataById($id);
        $this->load->view('admin/new_properties/edit', $data);
    }
    ...
    public function update() {
        ...
        if ($this->form_validation->run() === FALSE){
            ...
            $this->new_properties_model->update($data);
    ...
    public function photo_edit($id)
    {
        $data["photos"] = $this->new_properties_model->getDataByPhoto($id);
        $data['insert_id'] = $id;
        $this->load->view('admin/new_properties/photo_edit', $data);
    }


    public function photo_remove($id)
    {
        $this->new_properties_model->photo_remove($id);
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function changeStatusActive($id) {
        $this->new_properties_model->changeStatusActive($id);
        redirect($_SERVER['HTTP_REFERER']);
    }

    public function changeStatusInactive($id) {
        $this->new_properties_model->changeStatusInactive($id);
        redirect($_SERVER['HTTP_REFERER']);
    }
