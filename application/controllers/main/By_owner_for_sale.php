...
class By_owner_for_sale extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model('admin/by_owner_for_sale_model');
	...
	{
        $config = [
            'base_url' => base_url('main/by_owner_for_sale/index'),
            'total_rows' => $this->by_owner_for_sale_model->count_all(),
            'per_page' => 50,
        ...
        $data["for_sales"] = $this->by_owner_for_sale_model->getAll($config['per_page'], $offset);
        $data["count_all"] = $this->by_owner_for_sale_model->count_all();
		$this->load->view('admin/by_owner_for_sale/index', $data);
	}
    ...
    public function create()
    {
        ...
        $data['type'] = 'by_owner_for_sale';
        $this->load->view('admin/by_owner_for_sale/create', $data);
    }
    ...
    public function save() {
        ...
        if ($this->form_validation->run() === FALSE){
            ...
            $data["insert_id"] =  $this->by_owner_for_sale_model->insert($data);
            $this->session->set_flashdata('success', 'Added Successfully');
            $this->load->view('admin/by_owner_for_sale/upload_file', $data);
        }
    ...
    {
        $data["detail"] = $this->by_owner_for_sale_model->getDataById($id);
        $data["photos"] = $this->by_owner_for_sale_model->getDataByPhoto($id);
        $data["company_data"] = $this->company_model->get_by_company_data();
        $this->load->view('admin/by_owner_for_sale/detail', $data);
    }

    public function destroy($id)
    {
        $this->by_owner_for_sale_model->destroy_properties($id);
        $this->by_owner_for_sale_model->destroy_propertie_photo($id);
    ...
    public function edit($id)
    {
        ...
        $data["detail"] = $this->by_owner_for_sale_model->getDataById($id);
        $this->load->view('admin/by_owner_for_sale/edit', $data);
    }
    ...
    public function update() {
        ...
        if ($this->form_validation->run() === FALSE){
            ...
            $this->by_owner_for_sale_model->update($data);
    ...
    public function photo_edit($id)
    {
        $data["photos"] = $this->by_owner_for_sale_model->getDataByPhoto($id);
        $data['insert_id'] = $id;
        $this->load->view('admin/by_owner_for_sale/photo_edit', $data);
    }


    public function photo_remove($id)
    {
        $this->by_owner_for_sale_model->photo_remove($id);
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function changeStatusActive($id) {
        $this->by_owner_for_sale_model->changeStatusActive($id);
        redirect($_SERVER['HTTP_REFERER']);
    }

    public function changeStatusInactive($id) {
        $this->by_owner_for_sale_model->changeStatusInactive($id);
        redirect($_SERVER['HTTP_REFERER']);
    }
