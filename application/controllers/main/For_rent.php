...
class For_rent extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/for_rent_model');
    ...
    {
        $config = [
            'base_url' => base_url('main/for_rent/index'),
            'total_rows' => $this->for_rent_model->count_all(),
            'per_page' => 50,
        ...
        $data["for_sales"] = $this->for_rent_model->getAll($config['per_page'], $offset);
        $data["count_all"] = $this->for_rent_model->count_all();
        $this->load->view('admin/for_rent/index', $data);
    }
    ...
    public function create()
    {
        ...
        $data['type'] = 'for_rent';
        $this->load->view('admin/for_rent/create', $data);
    }
    ...
    public function save()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $data["insert_id"] =  $this->for_rent_model->insert($data);
            $this->session->set_flashdata('success', 'Added Successfully');
            $this->load->view('admin/for_rent/upload_file', $data);
        }
    }
    ...
    public function update()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $this->for_rent_model->update($data);
    ...
    {
        $data["detail"] = $this->for_rent_model->getDataById($id);
        $data["photos"] = $this->for_rent_model->getDataByPhoto($id);
        $data["company_data"] = $this->company_model->get_by_company_data();
        $this->load->view('admin/for_rent/detail', $data);
    }

    public function destroy($id)
    {
        $this->for_rent_model->destroy_properties($id);
        $this->for_rent_model->destroy_propertie_photo($id);
        redirect(base_url('main/for_rent'));
    }
    ...
    public function edit($id)
    {
        ...
        $data["detail"] = $this->for_rent_model->getDataById($id);
        $this->load->view('admin/for_rent/edit', $data);
    }
    ...
    public function photo_edit($id)
    {
        $data["photos"] = $this->for_rent_model->getDataByPhoto($id);
        $data['insert_id'] = $id;
        $this->load->view('admin/for_rent/photo_edit', $data);
    }


    public function photo_remove($id)
    {
        $this->for_rent_model->photo_remove($id);
        redirect($_SERVER['HTTP_REFERER']);
    }
    ...
    public function single_upload_video()
    {
        $config['upload_path']          = './uploads/';
        $config['allowed_types']        = 'mp4|avi|flv|wmv|mpg';
    ...
    {
        $this->for_rent_model->changeStatusActive($id);
        redirect($_SERVER['HTTP_REFERER']);
    }

    public function changeStatusInactive($id)
    {
        $this->for_rent_model->changeStatusInactive($id);
        redirect($_SERVER['HTTP_REFERER']);
    }
}
